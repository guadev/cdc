  ...
  if($pass1 == $pass2) {
    ...
    if(!empty(trim($firstname)) && !empty(trim($lastname)) && !empty(trim($username)) && !empty(trim($email)) && !empty(trim($password))
      ...
      if(register($firstname, $lastname, $username, $email, $password, $identitas, $noIdentitas, $alamat, $noHP, $asal)) {
        $pesan = '<div class="alert alert-success" role="alert">Berhasil mendaftar! Silahkan <a href="login.php">Masuk</a> untuk melanjutkan</div>';
      } else {
        $pesan = '<div class="alert alert-warning" role="alert">Username telah digunakan. Silahkan pilih username yang lain.</div>';
      }
    } else {
            ...
            <div class="col-md-offset-7 col-md-5">
              <div class="col-md-12 text-box">
                <div class="col-md-6 no-padding">
                  <input type="text" name="firstname" min="4" placeholder="Nama Depan" required/>
                </div>
                <div class="col-md-6 no-padding">
                  <input type="text" name="lastname" min="4" placeholder="Nama Belakang" required/>
                </div>
                <input type="text" name="username" min="5" placeholder="Username" required/>
                ...
                <div class="col-md-9 col-xs-9 col-sm-9 text-box no-padding">
                  <input type="text" name="noIdentitas" min="8" placeholder="Nomor Identitas" required>
                </div>
              <input type="text" name="alamat" min="5" placeholder="Alamat" required/>
              <label class="text-warning">*Wajib menggunakan nomor yang valid</label>
              <input type="text" name="no_hp" min="11" placeholder="No. HP" required/>
              <label class="text-warning">*Jika masih mahasiswa/pelajar</label>
              <input type="text" name="asal" placeholder="Universitas/sekolah" />
